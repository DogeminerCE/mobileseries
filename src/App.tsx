...
export default function App() {
  ...
  const totalSeriesEarnings = useMemo(() => {
    ...
    players.forEach(p => {
      if (p.events) {
        sum += p.events
          .filter(e => ['series', 'heats', 'qualifier'].includes(e.category || 'series'))
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end mb-8 gap-8">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 mt-2">
              <Link 
                to="/drop-map"
                className="flex items-center gap-2 px-4 py-2 bg-[#FCE14B]/10 border border-[#FCE14B]/30 text-[#FCE14B] hover:bg-[#FCE14B] hover:text-black transition-all font-black italic uppercase text-[10px] tracking-tighter w-fit group"
              >
